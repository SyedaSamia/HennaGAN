...
!ls "/content/drive/Your_Drive"

# Root directory for dataset
dataset_root = 'drive/path_to_dataset/'
...
optimizerD = optim.Adam(netD.parameters(), lr=leraning_rate, betas=(beta1, 0.999))
optimizerG = optim.Adam(netG.parameters(), lr=leraning_rate, betas=(beta1, 0.999))
...
#                   % (epoch, num_epochs, i, len(dataloader),
        ...
        if (iter % 500 == 0) or ((epoch == epochs - 1) and (i == len(dataloader) - 1)):
            with torch.no_grad():
...
"""Save this fragment in the drive using plt.save()"""

plt.figure(figsize=(10,5))
plt.title("Generator and Discriminator Loss During Training")
plt.plot(G_losses,label="G",color='magenta')
plt.plot(D_losses,label="D",color='blue')
...
ani.save('drive/My Drive/BachelorThesis/animationHennaGAN1.mp4', fps=30)
...
"""Try to save all the images from the img_list """
